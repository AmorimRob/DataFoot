...
public class AtletaDao extends HibernateDAO {
	...
	public List<Atleta> pesquisarAtletas(String nome, String categoria,
			String guidClube) {
		...
		try {
			...
			Query query = sessao
					.createSQLQuery(
							"SELECT atl.ATL_GUID_ATLETA, atl.ATL_NOME,  atl.ATL_APELIDO, cl.CL_NOME_CLUBE, "
	...
	public Atleta buscarAtletaPorGuid(String guidAtleta) {
		...
		try {
			sessao = HibernateUtil.getSessionFactory();
			consulta = sessao
					.createQuery("from Atleta a where a.guidAtleta = :ga");
	...
	@SuppressWarnings("unchecked")
	public List<Atleta> pesquisarAtletasPorClube(String guidClube) {
		List<Atleta> resultado = new ArrayList<Atleta>();
			...
			Query query = sessao
					.createSQLQuery(
							"SELECT atl.ATL_GUID_ATLETA, atl.ATL_NOME,  atl.ATL_APELIDO, cl.CL_NOME_CLUBE, "
									...
									.concat("LEFT JOIN df_posicao pos ON atl.ATL_GUID_POSICAO_ORIGINAL = pos.POS_GUID_POSICAO ")
									.concat("LEFT JOIN df_clube cl ON atl.ATL_GUID_CLUBE = cl.CL_GUID_CLUBE ")
									.concat("WHERE atl.ATL_GUID_CLUBE = :gc"))
